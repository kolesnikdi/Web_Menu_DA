...
key = '(?P<code>[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12})'

urlpatterns = [
    path('registration/', RegisterTryView.as_view(), name='user_reg'),
    # Always open regex with ^ and close with $. This will prevent another regex from overlapping
    re_path(r'^registration/{}/$'.format(key), RegisterConfirmView.as_view(), name='registration_confirm'),
    ...
    re_path(r'^djangofunction/{}/$'.format(key), django_register_confirm_function, name='djangofunction_confirm' ),
]
